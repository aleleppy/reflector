...
import {
  CrasController_createBody,
  type CrasInterface,
  Cras,
} from "./cras.schema.svelte";

export abstract class CrasModule {
  loading = $state<boolean>(false);
  forms = $state({
    create: new CrasController_createBody(),
  });
  ...
  private buildForms(isEmpty: boolean) {
    ...
    return { create: new CrasController_createBody({ empty: true }) };
  }

  /**  */
  protected async _createRun(params?: {
    behavior?: Behavior<CrasInterface, ApiErrorResponse>;
  }) {
    ...
    try {
      const response = await api.post<CrasInterface>({
        endpoint,
        data,
      });
      ...
      return { ok: true, data: new Cras({ data: response }) };
    } catch (e) {
