...
function isNestedDto(v: unknown): v is Record<string, unknown> {
  return (
    v != null &&
    typeof v === "object" &&
    typeof (v as { bundle?: unknown }).bundle === "function"
  );
}
  ...
  for (const [key, v] of Object.entries(schema)) {
    if (isBuildedInput(v)) {
      ...
      for (const item of v) {
        ...
        else if (isNestedDto(item))
          forEachBuildedInput(item, fn, skipOptionalEmpty);
      }
...
export function validateForm(schema: object): boolean {
  ...
  forEachBuildedInput(
    ...
    (input) => {
      input.touch();
      const fieldValid = isInputEmpty(input)
        ? !input.required
        : input.validate() === null;
...
export function bundleStrict<T extends Record<string, unknown>>(
  ...
  [K in keyof T as Exclude<T[K], undefined> extends never ? never : K]: Exclude<
    T[K],
    undefined
  >;
};
export function bundleStrict(
...
function isBuildedInput(v: unknown): v is BuildedInput<unknown> {
  return (
    v != null &&
    typeof v === "object" &&
    (v as { kind?: unknown }).kind === "builded"
  );
}
